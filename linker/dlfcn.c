...
void *linker_dlopen(const char *filename, int flag)
{
	...
	if(!is_systemlib(filename)) {
#endif
		ret = find_library(filename);
		if (unlikely(ret == NULL)) {
			//set_dlerror(DL_ERR_CANNOT_LOAD_LIBRARY);
			DL_ERR("[x][%s:%d] can not load library %s!", FILE, LINE, filename);
		} else {
			ret->refcount++;
		}
#ifndef TRACE_SYSTEM_LIB
			...
			DEBUG("[x][%5s:%d] %s is system lib. load it. handler 0x%8x ",
					FILE, LINE, filename, (u4)ret);
	}
#endif
...
#ifndef ARM_SANDBOX
/* yajin: we do not need this in our sandbox! */
